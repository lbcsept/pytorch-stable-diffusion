...
class VAE_Encoder(nn.Sequential):
    ...
    def __init__(self, input_channels:int=3, vae_dim:int=512, latent_dim:int=8,
        ...
        super().__init__(
            ...
            nn.Conv2d(int(vae_dim/4), int(vae_dim/4), kernel_size=3, stride=2, padding=0),
            ...
            nn.Conv2d(int(vae_dim/2), int(vae_dim/2), kernel_size=3, stride=2, padding=0),
...
if __name__ == "__main__":
    ...
    print(x.shape)    
    model = VAE_Encoder(input_channels=input_channels, vae_dim=vae_dim, latent_dim=latent_dim)
    y = model(x, noise)
    print(y.shape)